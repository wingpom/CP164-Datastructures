...
class BST:
    ...
    def __eq__(self, bst2):
        return self.equality_aux(self._root, bst2._root)
        
    def equality_aux(self, node1, node2):
        flag_identical = False
        if not node1 and not node2:
            flag_identical = True
        elif node1 and node2  and node1._data == node2._data:
            flag_identical = self.equality_aux(node1._left, node2._left) and self.equality_aux(node1._right, node2._right)     
    ...
    def is_valid(self):
        ...
        if self._root is None:
            return flag_valid
        flag_valid = self._is_valid_aux(self._root, flag_valid)
        return flag_valid
        
    def _is_valid_aux(self, node, flag_valid):
        if node is not None:
            if node._right is not None:
            ...
            if node._left is not None:
                if node._data < node._left._data:
                    flag_valid = False
            flag_valid = self._is_valid_aux(node._left, flag_valid)
            flag_valid = self._is_valid_aux(node._right, flag_valid)
        return flag_valid
    
    def reflect_vertical(self):
        return self._reflect_vertical_aux(self._root)
    ...
    def satisfy_property1(self):
        ...
        if self._root._left is None and self._root._right is None:
            ...
            return self._satisfy_property1_aux(self._root) >= 0
    
    def _satisfy_property1_aux(self, node):
        if node is None:
            flag_balanced = 0
        else:
            left = self._satisfy_property1_aux(node._left)
            right = self._satisfy_property1_aux(node._right)
            if left < 0 or right < 0 or abs(left - right) > 1:
                flag_balanced = -1
            else:
                flag_balanced = max(left, right) + 1
        return flag_balanced

    def satisfy_property2(self):
        return self._satisfy_property2_aux(self._root)

    def _satisfy_property2_aux(self, node):
        flag_satisfy_property2 = False
        if node is None:
            return True
        if node._left is None and node._right is None:
            flag_satisfy_property2 = True
        if node._left is not None and node._right is not None:
            return self._satisfy_property2_aux(node._left) and self._satisfy_property2_aux(node._right)
        return flag_satisfy_property2
